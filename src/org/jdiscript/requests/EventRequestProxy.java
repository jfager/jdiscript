...
public class EventRequestProxy implements InvocationHandler {
    ...
    /**
     * Convenience method for creating a new dynamic proxy instance using an
     * EventRequestProxy.
     *
     * @param <T>
     * @param origRequest The EventRequest to proxy.
     * @param iface The interface to implement. This should be one of the
     * {@link JDIScriptEventRequest} subinterfaces.
     * @return A dynamic proxy instance implementing the given interface using
     * an EventRequestProxy.
     */
    @SuppressWarnings("unchecked")
    public static <T> T proxy(EventRequest origRequest, Class<T> iface) {
        return (T) Proxy.newProxyInstance(iface.getClassLoader(), new Class<?>[]{iface}, new EventRequestProxy(origRequest));
    }
     ...
     * <p>
     * For invocations of addHandler, add the given DebugEventHandler according
     * to jdiscript's handler conventions
